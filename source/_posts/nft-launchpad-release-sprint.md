...
三个月前,我给这个项目写了一篇文章,讲它的合约设计:六种铸造模式、Gas 优化、安全防护。那些内容现在依然成立。但这篇文章讲的是另一件事:**一个功能完整的项目,离"可以发布"还有十五个 issue。**

写代码的时候,我们会自然地把"能跑"和"正确"混在一起。CI 是纠正这种混淆的工具——但它自己也会骗人:藏在子目录里,带着吞错误的开关,假装跑过。

发布冲刺做的最重要的一件事,不是修了十五个问题,而是把"我不知道"变成了"我知道"。

issue #1 到 #17 还挂在列表上,前三个的修复 PR 已经开好,等着合并。main 分支还是旧的目录结构,第一次真正的 CI 运行,会发生在合并之后。

那台 CI 跑起来的那天,仓库才算从 `scaffold-alchemy-main` 的壳里孵出来。
